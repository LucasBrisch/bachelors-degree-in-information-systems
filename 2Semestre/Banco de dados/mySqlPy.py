...
def selectAllRows():
      clearEntries()
      conexao = mysql.connector.connect(host='localhost', user='root', password='PUC@1234', database='DB_Python', port = 3305)
...
def insert():
      ...
      if(id == "" or  nome == "" or cel == "" or log == "" or pwd == ""):
            tk.messagebox.showinfo("ALERT", "Fill in your [id] [nome], [celular], [login] and [senha].")
      else:
            try:
                  conexao = mysql.connector.connect(host='localhost', user='root', password='PUC@1234', database='DB_Python', port = 3305)
